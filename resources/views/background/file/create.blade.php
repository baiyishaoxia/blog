...
@section('js')
    ...
    @endsection
    @section('content')
    {{Form::open(['url'=>URL::action('Background\FileController@postCreate'),'id'=>'form1'])}}
            <!--导航栏-->
    <div class="location">
        <a href="javascript:history.back(-1);" class="back"><i></i><span>返回上一页</span></a>
        <a href="{{URL::action('Admin\IndexController@info')}}" class="home"><i></i><span>首页</span></a>
        <i class="arrow"></i>
        <a href="{{URL::action('Background\FileController@getList')}}"><span>上传配置管理</span></a>
        <i class="arrow"></i>
        <span>添加配置</span>
    </div>
        ...
        <dl>
            ...
            <dd>
                <div class="rule-single-select">
                    {{Form::select('type',\App\Http\Model\Background\File::type())}}
                </div>
            </dd>
        </dl>
        <dl>
