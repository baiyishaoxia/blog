    ...
    <div class="page bk_content" style="top: 0;">
        <div class="weui_cells weui_cells_checkbox">
            @foreach($cart_items as $cart_item)
                <label class="weui_cell weui_check_label" for="{{$cart_item->product->art_id}}">
                    <div class="weui_cell_hd" style="width: 23px;">
                        <input type="checkbox" class="weui_check" name="cart_item" id="{{$cart_item->product->art_id}}" checked="checked">
                        <i class="weui_icon_checked"></i>
                    </div>
                    <div class="weui_cell_bd weui_cell_primary">
                        <div style="position: relative;">
                            <img class="bk_preview" src="{{Storage::url($cart_item->product->art_thumb)}}" class="m3_preview" onclick="_toProduct({{$cart_item->product->art_id}});"/>
                            <div style="position: absolute; left: 100px; right: 0; top: 0">
                                <p>{{$cart_item->product->art_title}}</p>
                                <p class="bk_time" style="margin-top: 15px;">数量: <span class="bk_summary">x{{$cart_item->count}}</span></p>
                                <p class="bk_time">总计: <span class="bk_price">￥{{$cart_item->product->art_order * $cart_item->count}}</span></p>
                            </div>
                        </div>
                    </div>
                </label>
...
@section('my-js')
    <script type="text/javascript">
        ...
        function _onDelete() {
            ...
            $.ajax({
                type: "GET",
                url: '{{URL::action('Admin\MobileApi\CartController@getDelCart')}}',
                dataType: 'json',
                cache: false,
