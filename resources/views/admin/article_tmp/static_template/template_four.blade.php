...
<head>
    <meta charset="utf-8" />
    <title>{{isset($match['title'])?$match['title']:"模板4"}}</title>
    @include('admin.article_tmp.static_template.layout')
    
</head>
<body>
@if(isset($match))
    <div class="mould-template2">
        ...
        @else
            <div class="mould-banners">
                <a href="javascript:void(0)"><img src="{{$match['images']?Storage::url($match['images'][0]['file_url']):URL::asset('/admin/tmp/images/mode5.png')}}"></a>
            </div>
        @endif
        @if(isset($sign_up))
...
@else
    <div class="mould-template2 mould-template-view">
        <div class="banners" style="background: url(/admin/tmp/images/mode5.png) no-repeat center top">
            <div class="w1200">
...
<script>
    ...
    //定位导航
    var navFix = $(".mould-template2 .mould-header");
    ...
    $('.mould-template2 .mould-header').navScroll({
        mobileDropdown: true,
        scrollSpy: true
    });
