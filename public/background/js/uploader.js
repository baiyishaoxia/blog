$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        sendurl: null, //发送地址
        filetypes: "*", //文件类型
        filesize: "3666666666", //文件大小
        btntext: "浏览...", //上传按钮的文字
        swf: null //SWF上传控件相对地址
    };
    //初始化上传控件
    $.fn.InitUploader = function (b,type_file=null) {
        var fun = function (parentObj) {
            ...
            uploader.on('uploadSuccess', function (file, data){
                ...
                if (type_file == 1){
                    console.log(data);
                    var html = '<dl><dt>版本号</dt>' + "<input type='text' name='Version' class='input normal'></dl>"+
                        '<dl><dt>适应系统</dt>' + "<input type='text' name='System' class='input normal'></dl>"+
                        '<dl><dt>版本附件</dt>' + "<input type='text' name='Path' class='input normal' value='" + data.path +"'></dl>"+
                        '<dl><dt>升级日志</dt>' + "<textarea name='Log' class='input normal'></textarea></dl>";
                    $('#tab2').find('.cc').prepend(html);
                }
                uploader.removeFile(file); //从队列中移除
            });
